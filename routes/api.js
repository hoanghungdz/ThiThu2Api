...
router.get('/get-list-sinhvien', async (req, res) => {
    try {
        const data = await SinhVienModels.find().populate();
        res.json({
            "status": 200,
...
router.post('/add-sinhvien', async (req, res) => {
    ...
    try {
        ...
        const newSinhVien = new SinhVienModels({
        ...
        const result = (await newSinhVien.save()).populate(); //Thêm vào database
...
// tim kiếm
router.get('/search-sinhvien-by-name', async (req, res) => {
    const { ten } = req.query;
    try {
        const data = await SinhVienModels.find({
...
router.delete('/delete-sinhvien-by-id/:id', async (req, res) => {
    try {
        const { id } = req.params
        const result = await SinhVienModels.findByIdAndDelete(id);
        if (result) {
            res.json({
                "status": 200,
...
router.put('/update-sinhvien-by-id/:id', async (req, res) => {
    try {
        ...
        console.log(id);
        const data = req.body
        const updateSinhVien = await SinhVienModels.findById(id)
        // const result = await updateDistributor.save();

        let result = null;
        if (updateSinhVien) {
            updateSinhVien.ma_sinh_vien = data.ma_sinh_vien ?? updateSinhVien.ma_sinh_vien,
                updateSinhVien.ten = data.ten ?? updateSinhVien.ten,
                updateSinhVien.gioi_tinh = data.gioi_tinh ?? updateSinhVien.gioi_tinh,
                updateSinhVien.ngay_sinh = data.ngay_sinh ?? updateSinhVien.ngay_sinh,
                updateSinhVien.khoa_hoc = data.khoa_hoc ?? updateSinhVien.khoa_hoc,
                updateSinhVien.lop = data.lop ?? updateSinhVien.lop,
